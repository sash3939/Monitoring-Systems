...
def get_metrics():
    ...
    # 2. Использование памяти (метрика из /proc/meminfo)
    with open('/proc/meminfo', 'r') as f:
        meminfo = {}
        for line in f:
            key, value = line.split(':')
            meminfo[key] = int(value.split()[0])  # Данные в kB
        metrics['mem_total'] = meminfo['MemTotal']
        metrics['mem_free'] = meminfo['MemFree']
        metrics['mem_available'] = meminfo['MemAvailable']
    
    # 3. Использование CPU (метрика из /proc/stat)
    ...
    # 4. Использование диска (метрика из os.statvfs для корневой директории)
    statvfs = os.statvfs('/')
